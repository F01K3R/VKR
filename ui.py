...
def process_file_wrapper(args):
    ...
    result = process_file((file_path, file_index))
    end_time = time.time()
    return {
        "file_path": file_path,
...
class ProcessingThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Создаём список задач с индексами
            tasks = [(file_path, i) for i, file_path in enumerate(self.file_paths)]
...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event):
        logger.debug("closeEvent: Начало закрытия пгшиложения")
